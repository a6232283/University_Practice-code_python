import affineCiper,pyperclip,detectEnglish,Cryptomath
...
def hackAffine(message):
    ...
    for key in range(len(affineCiper.SYMBOLS)**2):
        keyA=affineCiper.getKeyParts(key)[0]
        if Cryptomath.gcd(keyA,len(affineCiper.SYMBOLS))!=1:
            continue

        decryptedText=affineCiper.decryptMessage(key,message)
        if not SILENT_MODE:
